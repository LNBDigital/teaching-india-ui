...
const SuccessBtn = ({ active = false, content = "demo", ...props }) => (
  <button
    ...
    className={`rounded-md px-2 md:px-10 py-1 cursor-pointer ${
      active ? "bg-green-600 text-white" : "bg-transparent border border-black rounded-sm"
    }`}
  >
    {content}
  </button>
...
const TabSwitchBtn = ({ active = false, content = "demo", ...props }) => (
  <button
    {...props}
    className={`rounded-md px-2 md:px-10 py-1 cursor-pointer ${
      active ? "bg-lightBlue5 text-white" : "bg-transparent border border-black rounded-sm"
    }`}
  >
    {content}
  </button>
...
export default function LessonDetailPage() {
  ...
  return (
    <DashboardWrapper changeBg heading={`Lesson - ${currentLesson?.name || "Unknown"}`}>
      <div className="w-full flex flex-col gap-3 xl:flex-row xl:gap-15 relative">
<div className="relative w-full max-w-full xl:max-w-[70%] aspect-video overflow-hidden">
  <iframe
    className="absolute top-0 left-0 w-full h-full"
    src={currentLesson?.video || "https://www.youtube.com/embed/tgbNymZ7vqY"}
        ...
        <div>
          ...
          <ul className="flex flex-col gap-1 xl:gap-2">
            {cache?.lessons.map((item) => (
              <li key={item.id}>
                <button onClick={() => setCurrLessonId(item.id)} className="cursor-pointer text-md xl:text-xl">
                  {item.name}
                </button>
              </li>
            ))}
      ...
      <div className="my-10 w-full xl:w-[70%]">
        <div className="flex justify-between">
          <div className="inline-flex gap-4">
            <TabSwitchBtn active={!pdfShow} content="Summary" onClick={() => setPdfShow(false)} />
            <TabSwitchBtn active={pdfShow} content="PDF" onClick={() => setPdfShow(true)} />
          </div>

          {read ? (
            <SuccessBtn onClick={toggleReadStatus} content="Mark as Unread" />
          ) : (
            <SuccessBtn onClick={toggleReadStatus} active={false} content="Mark as Read" />
          )}
        </div>
